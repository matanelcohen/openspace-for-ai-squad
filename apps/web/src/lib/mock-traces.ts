...
function makeSpan(
  ...
  return {
    ...
    errorStack: status === 'error' ? `Error: ${name} failed\n    at execute (agent.ts:42)` : null,
    toolId: isTool ? `tool-${name.toLowerCase().replace(/\s+/g, '-')}` : null,
    timeToFirstToken: isLlm ? randomInt(50, 500) : null,
    streaming: isLlm ? Math.random() > 0.3 : null,
    inputBytes: inputData ? JSON.stringify(inputData).length : null,
    outputBytes: outputData ? JSON.stringify(outputData).length : null,
    events: status === 'error' ? [{ name: 'exception', timestamp: startTime + duration, attributes: { 'exception.message': `${name} failed` } }] : [],
    metadata: isTool
      ? {
          'tool.name': name,
